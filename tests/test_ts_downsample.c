...
MDB_TEST(ts_downsample_keeps_count_at_capacity) {
    ...
    ASSERT_EQ(microdb_ts_count(&g_db, "down", 0u, UINT32_MAX, &count), MICRODB_OK);
...
MDB_TEST(ts_downsample_merges_two_oldest_u32_samples) {
    ...
    out = (microdb_ts_sample_t *)malloc((size_t)capacity * sizeof(*out));
    ASSERT_EQ(out != NULL, 1);
    ASSERT_EQ(microdb_ts_query_buf(&g_db, "down", 0u, UINT32_MAX, out, capacity, &out_count), MICRODB_OK);
...
MDB_TEST(ts_downsample_merges_two_oldest_i32_samples) {
    ...
    out = (microdb_ts_sample_t *)malloc((size_t)capacity * sizeof(*out));
    ASSERT_EQ(out != NULL, 1);
    ASSERT_EQ(microdb_ts_query_buf(&g_db, "downi", 0u, UINT32_MAX, out, capacity, &out_count), MICRODB_OK);
    ASSERT_EQ(out_count, capacity);
    free(out);
}
...
MDB_TEST(ts_downsample_merges_two_oldest_f32_samples) {
    ...
    out = (microdb_ts_sample_t *)malloc((size_t)capacity * sizeof(*out));
    ASSERT_EQ(out != NULL, 1);
    ASSERT_EQ(microdb_ts_query_buf(&g_db, "downf", 0u, UINT32_MAX, out, capacity, &out_count), MICRODB_OK);
    ASSERT_EQ(out_count, capacity);
    free(out);
}
...
MDB_TEST(ts_downsample_raw_merges_oldest_pair_bytes) {
    ...
    out = (microdb_ts_sample_t *)malloc((size_t)capacity * sizeof(*out));
    ASSERT_EQ(out != NULL, 1);
    ASSERT_EQ(microdb_ts_query_buf(&g_db, "raw", 0u, UINT32_MAX, out, capacity, &out_count), MICRODB_OK);
    ASSERT_EQ(out_count, capacity);
