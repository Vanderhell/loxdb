...
static lox_err_t counting_sync(void *ctx) {
    g_sync_calls++;
    if (g_sync_prev != NULL) {
...
static void crash_drop_db_heap(lox_t *db) {
    if (lox_core_const(db)->magic == LOX_MAGIC) {
...
static void open_db_with_sync_mode(lox_t *db, lox_storage_t *storage, uint8_t wal_sync_mode) {
    ...
    g_now = 1000u;
    ASSERT_EQ(lox_port_posix_init(storage, g_path, 65536u), LOX_OK);
...
static void open_db(lox_t *db, lox_storage_t *storage) {
    open_db_with_sync_mode(db, storage, LOX_WAL_SYNC_ALWAYS);
}
...
static uint32_t read_u32(lox_storage_t *storage, uint32_t offset) {
    uint32_t value = 0u;
    if (storage->read(storage->ctx, offset, &value, sizeof(value)) != LOX_OK) {
        return 0u;
    }
    return value;
}

static uint8_t read_u8(lox_storage_t *storage, uint32_t offset) {
    uint8_t value = 0u;
    if (storage->read(storage->ctx, offset, &value, sizeof(value)) != LOX_OK) {
        return 0u;
    }
...
static uint32_t next_entry_offset(lox_storage_t *storage, uint32_t entry_offset) {
    ...
    if (storage->read(storage->ctx, entry_offset + 10u, &data_len, sizeof(data_len)) != LOX_OK) {
        return 0u;
    }
    return entry_offset + 16u + (((uint32_t)data_len + 3u) & ~3u);
}

static void reopen_after_power_loss(lox_t *db, lox_storage_t *storage) {
...
static void reopen_after_clean_shutdown(lox_t *db, lox_storage_t *storage) {
...
MDB_TEST(wal_sync_mode_controls_per_entry_sync) {
    ...
    open_db_with_sync_mode(&g_db, &g_storage, LOX_WAL_SYNC_ALWAYS);
    g_sync_prev = g_storage.sync;
    g_storage.sync = counting_sync;
    g_sync_calls = 0u;
    ASSERT_EQ(lox_kv_set(&g_db, "sync_always", &value, 1u, 0u), LOX_OK);
    ASSERT_EQ(g_sync_calls > 0u, 1);
    cleanup_handles(&g_db, &g_storage);

    open_db_with_sync_mode(&g_db, &g_storage, LOX_WAL_SYNC_FLUSH_ONLY);
    g_sync_prev = g_storage.sync;
    g_storage.sync = counting_sync;
    g_sync_calls = 0u;
    ASSERT_EQ(lox_kv_set(&g_db, "sync_flush_only", &value, 1u, 0u), LOX_OK);
...
MDB_TEST(wal_power_loss_after_kv_set_replays) {
    ...
    ASSERT_EQ(lox_kv_set(&g_db, "survive", &in, 1u, 0u), LOX_OK);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, "survive", &out, 1u, NULL), LOX_OK);
    ASSERT_EQ(out, 99u);
}
...
MDB_TEST(wal_power_loss_after_ts_insert_replays) {
    ...
    ASSERT_EQ(lox_ts_insert(&g_db, "i32", 55u, &value), LOX_OK);
    reopen_after_power_loss(&g_db, &g_storage);
...
MDB_TEST(wal_crc_corruption_discards_tail) {
    ...
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, "k1", &out, 1u, NULL), LOX_OK);
    ASSERT_EQ(out, 1u);
    ASSERT_EQ(lox_kv_get(&g_db, "k2", &out, 1u, NULL), LOX_ERR_NOT_FOUND);
}
...
MDB_TEST(wal_corrupt_header_magic_resets_clean) {
    ...
    ASSERT_EQ(g_storage.write(g_storage.ctx, 0u, &bad, sizeof(bad)), LOX_OK);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(read_u32(&g_storage, 0u), 0x4D44424Cu);
    ASSERT_EQ(lox_kv_get(&g_db, "missing", &out, 1u, NULL), LOX_ERR_NOT_FOUND);
}
...
MDB_TEST(wal_partial_entry_discarded) {
    ...
    ASSERT_EQ(g_storage.sync(g_storage.ctx), LOX_OK);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, "a1", &out, 1u, NULL), LOX_OK);
    ASSERT_EQ(lox_kv_get(&g_db, "a2", &out, 1u, NULL), LOX_ERR_NOT_FOUND);
}
...
MDB_TEST(wal_compaction_triggers_main_pages_update) {
    ...
    ASSERT_EQ(read_u32(&g_storage, 8u), 0u);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, last_key, &out, 1u, NULL), LOX_OK);
}
...
MDB_TEST(wal_kv_delete_replayed) {
    uint8_t value = 3u;
    ASSERT_EQ(lox_kv_set(&g_db, "gone", &value, 1u, 0u), LOX_OK);
...
MDB_TEST(wal_flush_compacts_and_persists) {
    ...
    ASSERT_EQ(lox_kv_set(&g_db, "flush", &value, 1u, 0u), LOX_OK);
    ASSERT_EQ(lox_flush(&g_db), LOX_OK);
    ASSERT_EQ(read_u32(&g_storage, 8u), 0u);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, "flush", &out, 1u, NULL), LOX_OK);
}

MDB_TEST(wal_deinit_compacts_before_free) {
    uint8_t value = 9u, out = 0u;
    ASSERT_EQ(lox_kv_set(&g_db, "bye", &value, 1u, 0u), LOX_OK);
    ASSERT_EQ(lox_deinit(&g_db), LOX_OK);
    ASSERT_EQ(read_u32(&g_storage, 8u), 0u);
    lox_port_posix_deinit(&g_storage);
    open_db(&g_db, &g_storage);
    ASSERT_EQ(lox_kv_get(&g_db, "bye", &out, 1u, NULL), LOX_OK);
}
...
MDB_TEST(wal_ram_only_mode_skips_storage) {
    ...
    ASSERT_EQ(lox_kv_set(&db, "ram", &value, 1u, 0u), LOX_OK);
    ASSERT_EQ(lox_kv_get(&db, "ram", &out, 1u, NULL), LOX_OK);
    ASSERT_EQ(lox_deinit(&db), LOX_OK);
}
...
MDB_TEST(wal_kv_ttl_persisted_after_reload) {
    ...
    ASSERT_EQ(lox_kv_set(&g_db, "ttl", &value, 1u, 10u), LOX_OK);
    reopen_after_clean_shutdown(&g_db, &g_storage);
    g_now = 1009u;
    ASSERT_EQ(lox_kv_get(&g_db, "ttl", &out, 1u, NULL), LOX_OK);
    g_now = 1011u;
    ASSERT_EQ(lox_kv_get(&g_db, "ttl", &out, 1u, NULL), LOX_ERR_EXPIRED);
}
...
MDB_TEST(wal_kv_purge_expired_persists_deletion) {
    ...
    ASSERT_EQ(lox_kv_set(&g_db, "purge_ttl", &value, 1u, 1u), LOX_OK);
    g_now = 2000u;
    ASSERT_EQ(lox_kv_purge_expired(&g_db), LOX_OK);
    reopen_after_power_loss(&g_db, &g_storage);
    g_now = 0u;
    ASSERT_EQ(lox_kv_get(&g_db, "purge_ttl", &out, 1u, NULL), LOX_ERR_NOT_FOUND);
}
...
MDB_TEST(wal_insufficient_storage_returns_storage) {
    ...
    ASSERT_EQ(lox_port_posix_init(&storage, small_path, 1024u), LOX_OK);
    memset(&cfg, 0, sizeof(cfg));
    cfg.storage = &storage;
    cfg.ram_kb = 32u;
    cfg.now = mock_now;
...
MDB_TEST(wal_sequence_advances_after_flush) {
    ...
    ASSERT_EQ(lox_kv_set(&g_db, "seq", &value, 1u, 0u), LOX_OK);
    ASSERT_EQ(lox_flush(&g_db), LOX_OK);
...
MDB_TEST(wal_table_metadata_persisted_before_row_replay) {
    lox_table_t *table = NULL;
    ASSERT_EQ(lox_ts_register(&g_db, "meta", LOX_TS_U32, 0u), LOX_OK);
    make_rel_table(&g_db, &table);
    reopen_after_power_loss(&g_db, &g_storage);
    ASSERT_EQ(lox_ts_last(&g_db, "meta", &(lox_ts_sample_t){0}), LOX_ERR_NOT_FOUND);
    ASSERT_EQ(lox_table_get(&g_db, "users", &table), LOX_OK);
}
...
MDB_TEST(wal_rel_iter_persists_insertion_order_after_reload) {
    ...
    reopen_after_clean_shutdown(&g_db, &g_storage);
    ASSERT_EQ(lox_table_get(&g_db, "users", &table), LOX_OK);
    ASSERT_EQ(lox_rel_iter(&g_db, table, rel_collect_ids, &ids), LOX_OK);
