#include "microtest.h"
#include "microdb.h"
#include "../port/ram/microdb_port_ram.h"
#include "../src/microdb_internal.h"

#include <stdio.h>
#include <string.h>

static microdb_t g_db;
static microdb_storage_t g_ram_storage;
static uint32_t g_mock_time = 0u;

typedef struct {
    uint32_t count;
    uint32_t stop_after;
} iter_ctx_t;

static microdb_timestamp_t mock_now(void) {
    return (microdb_timestamp_t)g_mock_time;
}

static microdb_core_t *test_core(void) {
    return microdb_core(&g_db);
}

static uint32_t test_kv_capacity(void) {
    return MICRODB_KV_MAX_KEYS - MICRODB_TXN_STAGE_KEYS;
}

static const microdb_kv_bucket_t *test_find_bucket(const char *key) {
    microdb_core_t *core = test_core();
    uint32_t i;

    for (i = 0; i < core->kv.bucket_count; ++i) {
        const microdb_kv_bucket_t *bucket = &core->kv.buckets[i];
        if (bucket->state == 1u && strcmp(bucket->key, key) == 0) {
            return bucket;
        }
    }

    return NULL;
}

static void test_make_key(char *buf, size_t buf_len, uint32_t index) {
    (void)snprintf(buf, buf_len, "k%03u", (unsigned)index);
}

static void setup_basic(void) {
    microdb_cfg_t cfg;

    ASSERT_EQ(microdb_port_ram_init(&g_ram_storage, 65536u), MICRODB_OK);
    memset(&cfg, 0, sizeof(cfg));
    cfg.storage = &g_ram_storage;
    cfg.ram_kb = 32u;
    cfg.now = NULL;
    ASSERT_EQ(microdb_init(&g_db, &cfg), MICRODB_OK);
}

static void setup_ttl(void) {
    microdb_cfg_t cfg;

    g_mock_time = 1000u;
    ASSERT_EQ(microdb_port_ram_init(&g_ram_storage, 65536u), MICRODB_OK);
    memset(&cfg, 0, sizeof(cfg));
    cfg.storage = &g_ram_storage;
    cfg.ram_kb = 32u;
    cfg.now = mock_now;
    ASSERT_EQ(microdb_init(&g_db, &cfg), MICRODB_OK);
}

static void teardown_db(void) {
    microdb_deinit(&g_db);
    microdb_port_ram_deinit(&g_ram_storage);
}

static bool count_iter_cb(const char *key, const void *val, size_t val_len, uint32_t ttl_remaining, void *ctx) {
    iter_ctx_t *iter = (iter_ctx_t *)ctx;

    (void)key;
    (void)val;
    (void)val_len;
    (void)ttl_remaining;
    iter->count++;
    return iter->count < iter->stop_after;
}

MDB_TEST(kv_set_and_get_basic) {
    uint8_t val_in[4] = { 0xDEu, 0xADu, 0xBEu, 0xEFu };
    uint8_t val_out[4] = { 0u, 0u, 0u, 0u };
    size_t out_len = 0u;

    ASSERT_EQ(microdb_kv_put(&g_db, "testkey", val_in, sizeof(val_in)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "testkey", val_out, sizeof(val_out), &out_len), MICRODB_OK);
    ASSERT_EQ(out_len, 4u);
    ASSERT_MEM_EQ(val_out, val_in, sizeof(val_in));
}

MDB_TEST(kv_get_nonexistent) {
    uint8_t buf[4] = { 0u, 0u, 0u, 0u };
    ASSERT_EQ(microdb_kv_get(&g_db, "nope", buf, sizeof(buf), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_delete_existing) {
    uint8_t value = 42u;
    uint8_t buf = 0u;

    ASSERT_EQ(microdb_kv_put(&g_db, "gone", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_del(&g_db, "gone"), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "gone", &buf, sizeof(buf), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_delete_nonexistent) {
    ASSERT_EQ(microdb_kv_del(&g_db, "ghost"), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_overwrite_existing_returns_latest_value) {
    uint8_t value_a[3] = { 1u, 2u, 3u };
    uint8_t value_b[5] = { 9u, 8u, 7u, 6u, 5u };
    uint8_t out[5] = { 0u, 0u, 0u, 0u, 0u };
    size_t out_len = 0u;

    ASSERT_EQ(microdb_kv_put(&g_db, "dup", value_a, sizeof(value_a)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "dup", value_b, sizeof(value_b)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "dup", out, sizeof(out), &out_len), MICRODB_OK);
    ASSERT_EQ(out_len, 5u);
    ASSERT_MEM_EQ(out, value_b, sizeof(value_b));
}

MDB_TEST(kv_overwrite_same_len_reuses_slot_without_growth) {
    uint8_t value_a[4] = { 1u, 1u, 1u, 1u };
    uint8_t value_b[4] = { 2u, 2u, 2u, 2u };
    const microdb_kv_bucket_t *before;
    const microdb_kv_bucket_t *after;
    uint32_t used_before;

    ASSERT_EQ(microdb_kv_put(&g_db, "same", value_a, sizeof(value_a)), MICRODB_OK);
    before = test_find_bucket("same");
    used_before = test_core()->kv.value_used;
    ASSERT_EQ(microdb_kv_put(&g_db, "same", value_b, sizeof(value_b)), MICRODB_OK);
    after = test_find_bucket("same");
    ASSERT_EQ(before->val_offset, after->val_offset);
    ASSERT_EQ(used_before, test_core()->kv.value_used);
}

MDB_TEST(kv_overwrite_larger_reuses_slot_and_grows_by_delta) {
    uint8_t value_a[2] = { 1u, 2u };
    uint8_t value_b[6] = { 9u, 8u, 7u, 6u, 5u, 4u };
    const microdb_kv_bucket_t *before;
    const microdb_kv_bucket_t *after;
    uint32_t used_before;

    ASSERT_EQ(microdb_kv_put(&g_db, "grow", value_a, sizeof(value_a)), MICRODB_OK);
    before = test_find_bucket("grow");
    used_before = test_core()->kv.value_used;
    ASSERT_EQ(microdb_kv_put(&g_db, "grow", value_b, sizeof(value_b)), MICRODB_OK);
    after = test_find_bucket("grow");
    ASSERT_EQ(before->val_offset, after->val_offset);
    ASSERT_EQ(used_before + 4u, test_core()->kv.value_used);
}

MDB_TEST(kv_key_exact_max_minus_one_is_valid) {
    char key[MICRODB_KV_KEY_MAX_LEN];
    uint8_t value = 1u;

    memset(key, 'a', sizeof(key));
    key[MICRODB_KV_KEY_MAX_LEN - 1u] = '\0';
    ASSERT_EQ(microdb_kv_put(&g_db, key, &value, sizeof(value)), MICRODB_OK);
}

MDB_TEST(kv_key_exact_max_is_invalid) {
    char key[MICRODB_KV_KEY_MAX_LEN + 1u];
    uint8_t value = 1u;

    memset(key, 'b', sizeof(key));
    key[MICRODB_KV_KEY_MAX_LEN] = '\0';
    ASSERT_EQ(microdb_kv_put(&g_db, key, &value, sizeof(value)), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_set_null_key_invalid) {
    uint8_t value = 1u;
    ASSERT_EQ(microdb_kv_put(&g_db, NULL, &value, sizeof(value)), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_set_empty_key_invalid) {
    uint8_t value = 1u;
    ASSERT_EQ(microdb_kv_put(&g_db, "", &value, sizeof(value)), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_get_null_key_invalid) {
    uint8_t value = 0u;
    ASSERT_EQ(microdb_kv_get(&g_db, NULL, &value, sizeof(value), NULL), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_get_empty_key_invalid) {
    uint8_t value = 0u;
    ASSERT_EQ(microdb_kv_get(&g_db, "", &value, sizeof(value), NULL), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_del_null_key_invalid) {
    ASSERT_EQ(microdb_kv_del(&g_db, NULL), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_del_empty_key_invalid) {
    ASSERT_EQ(microdb_kv_del(&g_db, ""), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_value_exact_max_len_ok) {
    uint8_t in[MICRODB_KV_VAL_MAX_LEN];
    uint8_t out[MICRODB_KV_VAL_MAX_LEN];
    size_t out_len = 0u;

    memset(in, 0xAB, sizeof(in));
    ASSERT_EQ(microdb_kv_put(&g_db, "maxval", in, sizeof(in)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "maxval", out, sizeof(out), &out_len), MICRODB_OK);
    ASSERT_EQ(out_len, (long long)sizeof(in));
    ASSERT_MEM_EQ(out, in, sizeof(in));
}

MDB_TEST(kv_value_overflow_is_rejected) {
    uint8_t big[MICRODB_KV_VAL_MAX_LEN + 1u];
    memset(big, 0xCC, sizeof(big));
    ASSERT_EQ(microdb_kv_put(&g_db, "toolarge", big, sizeof(big)), MICRODB_ERR_OVERFLOW);
}

MDB_TEST(kv_binary_all_zero_roundtrip) {
    uint8_t in[8] = { 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u };
    uint8_t out[8] = { 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u };
    ASSERT_EQ(microdb_kv_put(&g_db, "zeros", in, sizeof(in)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "zeros", out, sizeof(out), NULL), MICRODB_OK);
    ASSERT_MEM_EQ(out, in, sizeof(in));
}

MDB_TEST(kv_binary_all_ff_roundtrip) {
    uint8_t in[8];
    uint8_t out[8];

    memset(in, 0xFF, sizeof(in));
    memset(out, 0x00, sizeof(out));
    ASSERT_EQ(microdb_kv_put(&g_db, "ffs", in, sizeof(in)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "ffs", out, sizeof(out), NULL), MICRODB_OK);
    ASSERT_MEM_EQ(out, in, sizeof(in));
}

MDB_TEST(kv_ttl_zero_never_expires) {
    uint8_t value = 3u;

    ASSERT_EQ(microdb_kv_set(&g_db, "forever", &value, sizeof(value), 0u), MICRODB_OK);
    g_mock_time = 5000u;
    ASSERT_EQ(microdb_kv_exists(&g_db, "forever"), MICRODB_OK);
}

MDB_TEST(kv_ttl_one_expires_after_one_second) {
    uint8_t value = 7u;

    ASSERT_EQ(microdb_kv_set(&g_db, "ttl1", &value, sizeof(value), 1u), MICRODB_OK);
    g_mock_time = 1000u;
    ASSERT_EQ(microdb_kv_exists(&g_db, "ttl1"), MICRODB_OK);
    g_mock_time = 1001u;
    ASSERT_EQ(microdb_kv_exists(&g_db, "ttl1"), MICRODB_ERR_EXPIRED);
}

MDB_TEST(kv_get_expired_key_deletes_it) {
    uint8_t value = 4u;
    uint8_t out = 0u;

    ASSERT_EQ(microdb_kv_set(&g_db, "expire_get", &value, sizeof(value), 5u), MICRODB_OK);
    g_mock_time = 1006u;
    ASSERT_EQ(microdb_kv_get(&g_db, "expire_get", &out, sizeof(out), NULL), MICRODB_ERR_EXPIRED);
    ASSERT_EQ(microdb_kv_get(&g_db, "expire_get", &out, sizeof(out), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_exists_live_key_ok) {
    uint8_t value = 8u;
    ASSERT_EQ(microdb_kv_put(&g_db, "live", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_exists(&g_db, "live"), MICRODB_OK);
}

MDB_TEST(kv_exists_expired_key_returns_expired) {
    uint8_t value = 5u;

    ASSERT_EQ(microdb_kv_set(&g_db, "expire_exists", &value, sizeof(value), 2u), MICRODB_OK);
    g_mock_time = 1003u;
    ASSERT_EQ(microdb_kv_exists(&g_db, "expire_exists"), MICRODB_ERR_EXPIRED);
}

MDB_TEST(kv_purge_expired_removes_all_expired) {
    uint8_t value = 1u;
    iter_ctx_t ctx;

    ASSERT_EQ(microdb_kv_set(&g_db, "a", &value, sizeof(value), 1u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_set(&g_db, "b", &value, sizeof(value), 1u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_set(&g_db, "c", &value, sizeof(value), 0u), MICRODB_OK);
    g_mock_time = 1002u;
    ASSERT_EQ(microdb_kv_purge_expired(&g_db), MICRODB_OK);
    ctx.count = 0u;
    ctx.stop_after = 100u;
    ASSERT_EQ(microdb_kv_iter(&g_db, count_iter_cb, &ctx), MICRODB_OK);
    ASSERT_EQ(ctx.count, 1u);
}

MDB_TEST(kv_iter_visits_all_live_nonexpired_entries) {
    uint8_t value = 1u;
    iter_ctx_t ctx;

    ASSERT_EQ(microdb_kv_set(&g_db, "a", &value, sizeof(value), 0u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_set(&g_db, "b", &value, sizeof(value), 5u), MICRODB_OK);
    ASSERT_EQ(microdb_kv_set(&g_db, "c", &value, sizeof(value), 0u), MICRODB_OK);
    g_mock_time = 1006u;
    ctx.count = 0u;
    ctx.stop_after = 100u;
    ASSERT_EQ(microdb_kv_iter(&g_db, count_iter_cb, &ctx), MICRODB_OK);
    ASSERT_EQ(ctx.count, 2u);
}

MDB_TEST(kv_iter_stops_early_when_callback_returns_false) {
    uint8_t value = 1u;
    iter_ctx_t ctx;

    ASSERT_EQ(microdb_kv_put(&g_db, "a", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "b", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "c", &value, sizeof(value)), MICRODB_OK);
    ctx.count = 0u;
    ctx.stop_after = 2u;
    ASSERT_EQ(microdb_kv_iter(&g_db, count_iter_cb, &ctx), MICRODB_OK);
    ASSERT_EQ(ctx.count, 2u);
}

MDB_TEST(kv_clear_resets_live_count) {
    uint8_t value = 1u;

    ASSERT_EQ(microdb_kv_put(&g_db, "a", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "b", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_clear(&g_db), MICRODB_OK);
    ASSERT_EQ(test_core()->kv.entry_count, 0u);
}

MDB_TEST(kv_lru_evicts_oldest_entry_when_full) {
    uint8_t value = 9u;
    uint8_t out = 0u;
    char key[16];
    uint32_t i;

    for (i = 0; i < test_kv_capacity(); ++i) {
        test_make_key(key, sizeof(key), i);
        ASSERT_EQ(microdb_kv_put(&g_db, key, &value, sizeof(value)), MICRODB_OK);
    }

    ASSERT_EQ(microdb_kv_put(&g_db, "newest", &value, sizeof(value)), MICRODB_OK);
    test_make_key(key, sizeof(key), 0u);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_lru_get_refreshes_recentness) {
    uint8_t value = 9u;
    uint8_t out = 0u;
    char key[16];
    uint32_t i;

    for (i = 0; i < test_kv_capacity(); ++i) {
        test_make_key(key, sizeof(key), i);
        ASSERT_EQ(microdb_kv_put(&g_db, key, &value, sizeof(value)), MICRODB_OK);
    }

    test_make_key(key, sizeof(key), 0u);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "newest", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_OK);
    test_make_key(key, sizeof(key), 1u);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_lru_exists_refreshes_recentness) {
    uint8_t value = 9u;
    uint8_t out = 0u;
    char key[16];
    uint32_t i;

    for (i = 0; i < test_kv_capacity(); ++i) {
        test_make_key(key, sizeof(key), i);
        ASSERT_EQ(microdb_kv_put(&g_db, key, &value, sizeof(value)), MICRODB_OK);
    }

    test_make_key(key, sizeof(key), 0u);
    ASSERT_EQ(microdb_kv_exists(&g_db, key), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "newest", &value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_OK);
    test_make_key(key, sizeof(key), 1u);
    ASSERT_EQ(microdb_kv_get(&g_db, key, &out, sizeof(out), NULL), MICRODB_ERR_NOT_FOUND);
}

MDB_TEST(kv_overflow_reports_required_size) {
    uint8_t value_in[6] = { 1u, 2u, 3u, 4u, 5u, 6u };
    uint8_t value_out[2] = { 0u, 0u };
    size_t out_len = 0u;

    ASSERT_EQ(microdb_kv_put(&g_db, "smallbuf", value_in, sizeof(value_in)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_get(&g_db, "smallbuf", value_out, sizeof(value_out), &out_len), MICRODB_ERR_OVERFLOW);
    ASSERT_EQ(out_len, 6u);
}

MDB_TEST(kv_compaction_triggers_when_fragmentation_exceeds_half) {
    uint8_t value[8] = { 1u, 2u, 3u, 4u, 5u, 6u, 7u, 8u };
    uint32_t used_after_puts;

    ASSERT_EQ(microdb_kv_put(&g_db, "a", value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "b", value, sizeof(value)), MICRODB_OK);
    ASSERT_EQ(microdb_kv_put(&g_db, "c", value, sizeof(value)), MICRODB_OK);
    used_after_puts = test_core()->kv.value_used;
    ASSERT_EQ(used_after_puts, 24u);
    ASSERT_EQ(microdb_kv_del(&g_db, "a"), MICRODB_OK);
    ASSERT_EQ(test_core()->kv.value_used, 24u);
    ASSERT_EQ(microdb_kv_del(&g_db, "b"), MICRODB_OK);
    ASSERT_EQ(test_core()->kv.value_used, 8u);
}

MDB_TEST(kv_iter_null_callback_invalid) {
    ASSERT_EQ(microdb_kv_iter(&g_db, NULL, NULL), MICRODB_ERR_INVALID);
}

MDB_TEST(kv_set_null_value_invalid) {
    ASSERT_EQ(microdb_kv_set(&g_db, "nullval", NULL, 1u, 0u), MICRODB_ERR_INVALID);
}

int main(void) {
    MDB_RUN_TEST(setup_basic, teardown_db, kv_set_and_get_basic);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_get_nonexistent);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_delete_existing);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_delete_nonexistent);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_overwrite_existing_returns_latest_value);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_overwrite_same_len_reuses_slot_without_growth);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_overwrite_larger_reuses_slot_and_grows_by_delta);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_key_exact_max_minus_one_is_valid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_key_exact_max_is_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_set_null_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_set_empty_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_get_null_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_get_empty_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_del_null_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_del_empty_key_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_value_exact_max_len_ok);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_value_overflow_is_rejected);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_binary_all_zero_roundtrip);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_binary_all_ff_roundtrip);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_ttl_zero_never_expires);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_ttl_one_expires_after_one_second);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_get_expired_key_deletes_it);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_exists_live_key_ok);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_exists_expired_key_returns_expired);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_purge_expired_removes_all_expired);
    MDB_RUN_TEST(setup_ttl, teardown_db, kv_iter_visits_all_live_nonexpired_entries);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_iter_stops_early_when_callback_returns_false);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_clear_resets_live_count);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_lru_evicts_oldest_entry_when_full);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_lru_get_refreshes_recentness);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_lru_exists_refreshes_recentness);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_overflow_reports_required_size);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_compaction_triggers_when_fragmentation_exceeds_half);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_iter_null_callback_invalid);
    MDB_RUN_TEST(setup_basic, teardown_db, kv_set_null_value_invalid);
    return MDB_RESULT();
}
