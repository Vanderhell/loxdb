...
static void setup_db(void) {
    ...
    ASSERT_EQ(microdb_ts_register(&g_db, "s1", MICRODB_TS_U32, 0u), MICRODB_OK);
}
...
MDB_TEST(fuzz_import_parsers_do_not_crash) {
    static const char *valid_kv = "{\"format\":\"microdb.kv.v1\",\"items\":[{\"key\":\"k1\",\"ttl\":0,\"value_hex\":\"0102\"}]}";
    static const char *valid_ts = "{\"format\":\"microdb.ts.v1\",\"items\":[{\"stream\":\"s1\",\"type\":\"u32\",\"ts\":1,\"value_hex\":\"01000000\"}]}";
    static const char *valid_rel = "{\"format\":\"microdb.rel.v1\",\"items\":[]}";
    microdb_ie_options_t opts = microdb_ie_default_options();
    microdb_ie_ts_stream_desc_t ts_desc;
    uint32_t i;

    opts.skip_invalid_items = 1u;
    ts_desc.name = "s1";
    ts_desc.type = MICRODB_TS_U32;
    ts_desc.raw_size = 0u;
    ...
    for (i = 0u; i < 400u; ++i) {
        ...
        rc = microdb_ie_import_kv_json(&g_db, kv, &opts, &imported, &skipped);
        ASSERT_EQ((rc == MICRODB_OK || rc == MICRODB_ERR_INVALID || rc == MICRODB_ERR_OVERFLOW), 1);
        rc = microdb_ie_import_ts_json(&g_db, ts, &ts_desc, 1u, &opts, &imported, &skipped);
        ASSERT_EQ((rc == MICRODB_OK || rc == MICRODB_ERR_INVALID || rc == MICRODB_ERR_OVERFLOW), 1);
        rc = microdb_ie_import_rel_json(&g_db, rel, NULL, 0u, &opts, &imported, &skipped);
        ASSERT_EQ((rc == MICRODB_OK || rc == MICRODB_ERR_INVALID || rc == MICRODB_ERR_OVERFLOW), 1);
    }
}
