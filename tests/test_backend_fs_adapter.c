...
static lox_backend_fs_adapter_ctx_t g_adapter_ctx;
...
static void teardown_storage(void) {
    lox_backend_fs_adapter_deinit(&g_adapted_storage);
}

MDB_TEST(fs_adapter_default_policy_sync_is_explicit) {
    static const uint8_t payload[4] = { 1u, 2u, 3u, 4u };
    ASSERT_EQ(lox_backend_fs_adapter_init(&g_adapted_storage, &g_adapter_ctx, &g_raw_storage), LOX_OK);
...
MDB_TEST(fs_adapter_write_through_syncs_each_mutation) {
    ...
    e.sync_policy = LOX_BACKEND_FS_SYNC_POLICY_WRITE_THROUGH;

    ASSERT_EQ(lox_backend_fs_adapter_init_with_expectations(&g_adapted_storage, &g_adapter_ctx, &g_raw_storage, &e), LOX_OK);
    ...
    ASSERT_EQ(g_adapted_storage.erase(g_adapted_storage.ctx, 64u), LOX_OK);
...
MDB_TEST(fs_adapter_none_policy_skips_sync_calls) {
    lox_backend_fs_expectations_t e;
    lox_backend_fs_expectations_default(&e);
    e.sync_policy = LOX_BACKEND_FS_SYNC_POLICY_NONE;
...
MDB_TEST(fs_adapter_sync_probe_failure_returns_storage_error) {
    g_raw.sync_status = LOX_ERR_STORAGE;
    ASSERT_EQ(lox_backend_fs_adapter_init(&g_adapted_storage, &g_adapter_ctx, &g_raw_storage), LOX_ERR_STORAGE);
    ASSERT_EQ(g_raw.sync_calls, 1);
}
