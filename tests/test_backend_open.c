// SPDX-License-Identifier: MIT
#include "microtest.h"
#include "lox_backend_adapter.h"
#include "lox_backend_open.h"

#include <string.h>

int lox_backend_aligned_stub_register(void);
int lox_backend_nand_stub_register(void);
int lox_backend_emmc_stub_register(void);
int lox_backend_sd_stub_register(void);
int lox_backend_fs_stub_register(void);
int lox_backend_block_stub_register(void);

typedef struct {
    uint8_t mem[4096];
    uint32_t write_calls;
    uint32_t write_size;
    lox_err_t sync_status;
    uint32_t sync_calls;
} raw_ctx_t;

static raw_ctx_t g_raw;

static lox_err_t raw_read(void *ctx, uint32_t offset, void *buf, size_t len) {
    raw_ctx_t *raw = (raw_ctx_t *)ctx;
    if (raw == NULL || buf == NULL || ((size_t)offset + len) > sizeof(raw->mem)) {
        return LOX_ERR_STORAGE;
    }
    memcpy(buf, raw->mem + offset, len);
    return LOX_OK;
}

static lox_err_t raw_write(void *ctx, uint32_t offset, const void *buf, size_t len) {
    raw_ctx_t *raw = (raw_ctx_t *)ctx;
    if (raw == NULL || buf == NULL || ((size_t)offset + len) > sizeof(raw->mem)) {
        return LOX_ERR_STORAGE;
    }
    if (len == 0u) {
        return LOX_ERR_STORAGE;
    }
    if (raw->write_size == 0u) {
        return LOX_ERR_STORAGE;
    }
    if ((offset % raw->write_size) != 0u || (len % raw->write_size) != 0u) {
        return LOX_ERR_STORAGE;
    }
    memcpy(raw->mem + offset, buf, len);
    raw->write_calls++;
    return LOX_OK;
}

static lox_err_t raw_erase(void *ctx, uint32_t offset) {
    raw_ctx_t *raw = (raw_ctx_t *)ctx;
    if (raw == NULL || offset >= sizeof(raw->mem)) {
        return LOX_ERR_STORAGE;
    }
    return LOX_OK;
}

static lox_err_t raw_sync(void *ctx) {
    raw_ctx_t *raw = (raw_ctx_t *)ctx;
    if (raw == NULL) {
        return LOX_ERR_STORAGE;
    }
    raw->sync_calls++;
    return raw->sync_status;
}

static lox_storage_capability_t byte_capability(void) {
    lox_storage_capability_t cap;
    memset(&cap, 0, sizeof(cap));
    cap.backend_class = LOX_BACKEND_CLASS_BYTE;
    cap.minimal_write_unit = 1u;
    cap.erase_granularity = 256u;
    cap.atomic_write_granularity = 1u;
    cap.sync_semantics = LOX_SYNC_SEMANTICS_DURABLE_SYNC;
    return cap;
}

static lox_storage_capability_t fs_capability(lox_sync_semantics_t sync_semantics) {
    lox_storage_capability_t cap;
    memset(&cap, 0, sizeof(cap));
    cap.backend_class = LOX_BACKEND_CLASS_MANAGED;
    cap.minimal_write_unit = 1u;
    cap.erase_granularity = 512u;
    cap.atomic_write_granularity = 1u;
    cap.sync_semantics = sync_semantics;
    cap.is_managed = 1u;
    return cap;
}

static void setup_open(void) {
    memset(&g_raw, 0, sizeof(g_raw));
    memset(g_raw.mem, 0xFF, sizeof(g_raw.mem));
    g_raw.write_size = 16u;
    g_raw.sync_status = LOX_OK;
    lox_backend_registry_reset();
}

static void teardown_open(void) {
    lox_backend_registry_reset();
}

MDB_TEST(backend_open_unknown_backend_rejected) {
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 64u, 16u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    lox_err_t rc = lox_backend_open_prepare("missing", &raw, 1u, 0u, &session, &effective);
    ASSERT_EQ(rc, LOX_ERR_INVALID);
    ASSERT_EQ(effective == NULL, 1);
    ASSERT_EQ(session.last_decision.reason, LOX_BACKEND_REASON_BACKEND_NOT_REGISTERED);
}

MDB_TEST(backend_open_byte_backend_direct_passthrough) {
    g_raw.write_size = 1u;
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 256u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    lox_backend_adapter_t byte_adapter = { "byte_stub", { 0 } };
    byte_adapter.capability = byte_capability();

    ASSERT_EQ(lox_backend_registry_register(&byte_adapter), 0);
    ASSERT_EQ(lox_backend_open_prepare("byte_stub", &raw, 0u, 0u, &session, &effective), LOX_OK);
    ASSERT_EQ(effective == &raw, 1);
    ASSERT_EQ(session.using_aligned_adapter, 0);
    lox_backend_open_release(&session);
}

MDB_TEST(backend_open_aligned_requires_adapter_flag) {
    g_raw.write_size = 16u;
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 16u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;

    ASSERT_EQ(lox_backend_aligned_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("aligned_stub", &raw, 0u, 0u, &session, &effective), LOX_ERR_INVALID);
    ASSERT_EQ(session.last_decision.reason, LOX_BACKEND_REASON_MISSING_ALIGNED_ADAPTER);
}

MDB_TEST(backend_open_aligned_rejects_storage_contract_mismatch) {
    g_raw.write_size = 8u;
    {
        lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 64u, 8u, &g_raw };
        lox_backend_open_session_t session;
        lox_storage_t *effective = NULL;
        ASSERT_EQ(lox_backend_aligned_stub_register(), 0);
        ASSERT_EQ(lox_backend_open_prepare("aligned_stub", &raw, 1u, 0u, &session, &effective), LOX_ERR_INVALID);
        ASSERT_EQ(effective == NULL, 1);
        ASSERT_EQ(session.last_decision.reason, LOX_BACKEND_REASON_INVALID_STORAGE_CONTRACT);
    }
}

MDB_TEST(backend_open_aligned_uses_adapter_shim) {
    static const uint8_t payload[3] = { 0xA1u, 0xA2u, 0xA3u };
    g_raw.write_size = 16u;
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 16u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;

    ASSERT_EQ(lox_backend_aligned_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("aligned_stub", &raw, 1u, 0u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_aligned_adapter, 1);
    ASSERT_EQ(effective != NULL, 1);
    ASSERT_EQ(effective->write_size, 1);

    ASSERT_EQ(effective->write(effective->ctx, 5u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.write_calls, 1);
    ASSERT_EQ(g_raw.mem[5], 0xA1);
    ASSERT_EQ(g_raw.mem[7], 0xA3);

    lox_backend_open_release(&session);
    ASSERT_EQ(session.using_aligned_adapter, 0);
}

MDB_TEST(backend_open_managed_uses_managed_adapter) {
    static const uint8_t payload[3] = { 0xB1u, 0xB2u, 0xB3u };
    g_raw.write_size = 1u;
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;

    ASSERT_EQ(lox_backend_nand_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("nand_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_managed_adapter, 1);
    ASSERT_EQ(effective != NULL, 1);
    ASSERT_EQ(effective->write_size, 1);

    ASSERT_EQ(effective->write(effective->ctx, 5u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[5], 0xB1);
    ASSERT_EQ(g_raw.mem[7], 0xB3);
    ASSERT_EQ(g_raw.sync_calls, 1);

    lox_backend_open_release(&session);
    ASSERT_EQ(session.using_managed_adapter, 0);
}

MDB_TEST(backend_open_managed_fails_on_sync_probe_failure) {
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 1u;
    g_raw.sync_status = LOX_ERR_STORAGE;

    ASSERT_EQ(lox_backend_nand_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("nand_stub", &raw, 0u, 1u, &session, &effective), LOX_ERR_STORAGE);
    ASSERT_EQ(effective == NULL, 1);
    ASSERT_EQ(session.using_managed_adapter, 0);
    ASSERT_EQ(g_raw.sync_calls, 1);
}

MDB_TEST(backend_open_managed_requires_byte_write_contract) {
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 4u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 4u;

    ASSERT_EQ(lox_backend_nand_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("nand_stub", &raw, 0u, 1u, &session, &effective), LOX_ERR_INVALID);
    ASSERT_EQ(effective == NULL, 1);
    ASSERT_EQ(session.last_decision.reason, LOX_BACKEND_REASON_BYTE_WRITE_NOT_SUPPORTED);
}

MDB_TEST(backend_open_emmc_uses_managed_adapter) {
    static const uint8_t payload[2] = { 0xC1u, 0xC2u };
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 1u;

    ASSERT_EQ(lox_backend_emmc_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("emmc_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_managed_adapter, 1);
    ASSERT_EQ(effective != NULL, 1);
    ASSERT_EQ(effective->write(effective->ctx, 9u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[9], 0xC1);
    ASSERT_EQ(g_raw.mem[10], 0xC2);

    lox_backend_open_release(&session);
}

MDB_TEST(backend_open_sd_uses_managed_adapter) {
    static const uint8_t payload[2] = { 0xD1u, 0xD2u };
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 1u;

    ASSERT_EQ(lox_backend_sd_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("sd_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_managed_adapter, 1);
    ASSERT_EQ(effective != NULL, 1);
    ASSERT_EQ(effective->write(effective->ctx, 11u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[11], 0xD1);
    ASSERT_EQ(g_raw.mem[12], 0xD2);

    lox_backend_open_release(&session);
}

MDB_TEST(backend_open_managed_flush_only_uses_fs_adapter) {
    static const uint8_t payload[2] = { 0xE1u, 0xE2u };
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    lox_backend_adapter_t fs_adapter = { "fs_stub", { 0 } };
    g_raw.write_size = 1u;

    fs_adapter.capability = fs_capability(LOX_SYNC_SEMANTICS_FLUSH_ONLY);
    ASSERT_EQ(lox_backend_registry_register(&fs_adapter), 0);
    ASSERT_EQ(lox_backend_open_prepare("fs_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_fs_adapter, 1);
    ASSERT_EQ(session.using_managed_adapter, 0);
    ASSERT_EQ(effective != NULL, 1);
    ASSERT_EQ(effective->write(effective->ctx, 13u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[13], 0xE1);
    ASSERT_EQ(g_raw.mem[14], 0xE2);

    ASSERT_EQ(g_raw.sync_calls, 1);
    ASSERT_EQ(effective->sync(effective->ctx), LOX_OK);
    ASSERT_EQ(g_raw.sync_calls, 2);

    lox_backend_open_release(&session);
    ASSERT_EQ(session.using_fs_adapter, 0);
}

MDB_TEST(backend_open_fs_stub_uses_fs_adapter) {
    static const uint8_t payload[2] = { 0xF1u, 0xF2u };
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 4096u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 1u;

    ASSERT_EQ(lox_backend_fs_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("fs_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_fs_adapter, 1);
    ASSERT_EQ(session.using_managed_adapter, 0);
    ASSERT_EQ(effective != NULL, 1);

    ASSERT_EQ(effective->write(effective->ctx, 15u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[15], 0xF1);
    ASSERT_EQ(g_raw.mem[16], 0xF2);
    ASSERT_EQ(g_raw.sync_calls, 1);
    ASSERT_EQ(effective->sync(effective->ctx), LOX_OK);
    ASSERT_EQ(g_raw.sync_calls, 2);
    lox_backend_open_release(&session);
}

MDB_TEST(backend_open_block_stub_uses_fs_adapter_with_sync_none) {
    static const uint8_t payload[2] = { 0xA9u, 0xB9u };
    lox_storage_t raw = { raw_read, raw_write, raw_erase, raw_sync, (uint32_t)sizeof(g_raw.mem), 512u, 1u, &g_raw };
    lox_backend_open_session_t session;
    lox_storage_t *effective = NULL;
    g_raw.write_size = 1u;
    g_raw.sync_status = LOX_ERR_STORAGE;

    ASSERT_EQ(lox_backend_block_stub_register(), 0);
    ASSERT_EQ(lox_backend_open_prepare("block_stub", &raw, 0u, 1u, &session, &effective), LOX_OK);
    ASSERT_EQ(session.using_fs_adapter, 1);
    ASSERT_EQ(session.using_managed_adapter, 0);
    ASSERT_EQ(effective != NULL, 1);

    ASSERT_EQ(effective->write(effective->ctx, 21u, payload, sizeof(payload)), LOX_OK);
    ASSERT_EQ(g_raw.mem[21], 0xA9);
    ASSERT_EQ(g_raw.mem[22], 0xB9);
    ASSERT_EQ(g_raw.sync_calls, 0);
    ASSERT_EQ(effective->sync(effective->ctx), LOX_OK);
    ASSERT_EQ(g_raw.sync_calls, 0);
    lox_backend_open_release(&session);
}

int main(void) {
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_unknown_backend_rejected);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_byte_backend_direct_passthrough);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_aligned_requires_adapter_flag);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_aligned_rejects_storage_contract_mismatch);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_aligned_uses_adapter_shim);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_managed_uses_managed_adapter);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_managed_fails_on_sync_probe_failure);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_managed_requires_byte_write_contract);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_emmc_uses_managed_adapter);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_sd_uses_managed_adapter);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_managed_flush_only_uses_fs_adapter);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_fs_stub_uses_fs_adapter);
    MDB_RUN_TEST(setup_open, teardown_open, backend_open_block_stub_uses_fs_adapter_with_sync_none);
    return MDB_RESULT();
}
