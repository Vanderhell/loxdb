...
static microdb_storage_t g_st;

static void setup_db(void) {
    microdb_cfg_t cfg;
    ASSERT_EQ(microdb_port_ram_init(&g_st, 65536u), MICRODB_OK);
...
static void teardown_db(void) {
    microdb_deinit(&g_db);
    microdb_port_ram_deinit(&g_st);
}

MDB_TEST(json_u32_roundtrip) {
    uint32_t out = 0u;
    ASSERT_EQ(microdb_json_kv_set_u32(&g_db, "n", 123456u, 0u), MICRODB_OK);
    ASSERT_EQ(microdb_json_kv_get_u32(&g_db, "n", &out), MICRODB_OK);
    ASSERT_EQ(out, 123456u);
}

MDB_TEST(json_bool_roundtrip) {
    bool out = false;
    ASSERT_EQ(microdb_json_kv_set_bool(&g_db, "b", true, 0u), MICRODB_OK);
    ASSERT_EQ(microdb_json_kv_get_bool(&g_db, "b", &out), MICRODB_OK);
    ASSERT_EQ(out, 1);
}
...
MDB_TEST(json_cstr_roundtrip) {
    char out[64];
    size_t out_len = 0u;
    ASSERT_EQ(microdb_json_kv_set_cstr(&g_db, "s", "hello-json", 0u), MICRODB_OK);
    ASSERT_EQ(microdb_json_kv_get_cstr(&g_db, "s", out, sizeof(out), &out_len), MICRODB_OK);
...
MDB_TEST(json_record_encode_decode) {
    ...
    ASSERT_EQ(microdb_json_encode_kv_record("k\\\"x", in_val, sizeof(in_val), 77u, json, sizeof(json), &used), MICRODB_OK);
    ASSERT_GT(used, 0u);
    ASSERT_EQ(microdb_json_decode_kv_record(json, key, sizeof(key), out_val, sizeof(out_val), &out_val_len, &ttl), MICRODB_OK);
...
MDB_TEST(json_record_decode_invalid_rejected) {
    ...
    ASSERT_EQ(microdb_json_decode_kv_record(bad, key, sizeof(key), value, sizeof(value), &value_len, &ttl), MICRODB_ERR_INVALID);
}
