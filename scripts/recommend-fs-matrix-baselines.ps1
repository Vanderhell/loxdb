...
$files = Get-ChildItem -Path $InputDir -Recurse -File -Filter "fs_matrix_baseline_*.json"
if ($files.Count -eq 0) {
    throw "No baseline files found under '$InputDir' matching fs_matrix_baseline_*.json"
}
...
$result = @()
$mdLines = @()
$mdLines += "# FS Matrix Baseline Recommendations"
$mdLines += ""
