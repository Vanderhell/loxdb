...
$recommendJson = Join-Path $OutputDir "fs-matrix-baseline-recommendations.json"
$recommendMd = Join-Path $OutputDir "fs-matrix-baseline-recommendations.md"
$recommendLog = Join-Path $OutputDir "fs-matrix-baseline-recommendations.log"
$applyDryrunLog = Join-Path $OutputDir "fs-matrix-threshold-apply-dryrun.log"
$applyLog = Join-Path $OutputDir "fs-matrix-threshold-apply.log"
$candidatePresets = Join-Path $OutputDir "CMakePresets.candidate.json"
$candidateDiff = Join-Path $OutputDir "fs-matrix-threshold-presets.diff"
$finalSummary = Join-Path $OutputDir "fs-matrix-calibration-finalize-summary.md"

Write-Output "Generating recommendations from '$InputDir'..."
./scripts/recommend-fs-matrix-baselines.ps1 `
    -InputDir $InputDir `
    -Quantile $Quantile `
    -MarginPct $MarginPct `
...
$summary = @()
$summary += "# FS Matrix Calibration Finalize Summary"
$summary += ""
