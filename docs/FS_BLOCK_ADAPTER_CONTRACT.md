...
## Scope
- Applies to `microdb_backend_fs_adapter` path selected through `microdb_backend_open`.
...
- `backend_class == MANAGED` and `sync_semantics == DURABLE_SYNC`:
  - route to `microdb_backend_managed_adapter`.
- `backend_class == MANAGED` and `sync_semantics == FLUSH_ONLY`:
  - route to `microdb_backend_fs_adapter` with `EXPLICIT` policy.
- `backend_class == MANAGED` and `sync_semantics == NONE`:
  - route to `microdb_backend_fs_adapter` with `NONE` policy.

## Sync Policies
- `MICRODB_BACKEND_FS_SYNC_POLICY_EXPLICIT`
  - Writes/erases are forwarded to raw medium.
  - Durability point is explicit `sync`.
  - Mount-time sync probe is required by default.
- `MICRODB_BACKEND_FS_SYNC_POLICY_WRITE_THROUGH`
...
- `MICRODB_BACKEND_FS_SYNC_POLICY_NONE`
  - Adapter `sync` is a no-op (`MICRODB_OK`) and does not call raw `sync`.
...
- `WRITE_THROUGH`:
  - Mutations are durable once write/erase returns `MICRODB_OK`.
- `NONE`:
  - Adapter makes no durability claim for `sync`.
  - Persistence depends on raw backend behavior.

## Failure Semantics
- Invalid raw storage contract -> `MICRODB_ERR_INVALID`.
- Sync probe failure on mount (when enabled) -> `MICRODB_ERR_STORAGE`.
