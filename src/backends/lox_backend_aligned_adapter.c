...
static lox_err_t aligned_read(void *ctx, uint32_t offset, void *buf, size_t len) {
    lox_backend_aligned_adapter_ctx_t *adapter = (lox_backend_aligned_adapter_ctx_t *)ctx;
    lox_storage_t *raw;
    uint32_t unit;
    uint64_t pos;
    uint64_t end;
...
void lox_backend_aligned_adapter_deinit(lox_storage_t *out_storage) {
    lox_backend_aligned_adapter_ctx_t *adapter;

    if (out_storage == NULL || out_storage->ctx == NULL) {
        return;
    }

    adapter = (lox_backend_aligned_adapter_ctx_t *)out_storage->ctx;
