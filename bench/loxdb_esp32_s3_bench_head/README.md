...
1. `KV`: `lox_kv_put`, `lox_kv_get`, `lox_kv_del`
2. `TS`: `lox_ts_register`, `lox_ts_insert`, `lox_ts_query_buf`
3. `REL`: schema create + `lox_rel_insert` + `lox_rel_find`
4. `WAL`: WAL fill, `lox_compact`, `lox_inspect` validation
5. `Migration`: schema-version bump and `on_migrate` callback validation
6. `TXN`: `lox_txn_begin`, `lox_txn_commit`, `lox_txn_rollback`
...
If you need real flash-latency benchmarking, switch to `port/esp32/lox_port_esp32.*` with a partition label.

## Arduino Integration

This folder already includes local build helpers:

- `lox.h` (local copy of public header)
- `src/*.c` + `src/*.h` (local copies of loxdb sources)

This allows Arduino IDE builds without fragile `../../...` include paths.

If core files under `include/` or `src/` change in the repository, sync local copies in `bench/loxdb_esp32_s3_bench_head/`.
...
```text
loxdb ESP32-S3 terminal bench is ready.
...
- `stats`: print `lox_inspect()` snapshot
