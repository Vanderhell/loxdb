...
static lox_timestamp_t bench_now(void) { return (lox_timestamp_t)millis(); }

static lox_err_t st_read(void *ctx, uint32_t off, void *buf, size_t len) {
  bench_storage_ctx_t *s = (bench_storage_ctx_t *)ctx;
  if (s == NULL || s->bytes == NULL || buf == NULL || (off + len) > s->size) return LOX_ERR_INVALID;
...
static lox_err_t st_erase(void *ctx, uint32_t off) {
  bench_storage_ctx_t *s = (bench_storage_ctx_t *)ctx;
  if (s == NULL || s->bytes == NULL || off >= s->size || (off + BENCH_STORAGE_ERASE) > s->size) return LOX_ERR_INVALID;
...
static lox_err_t on_migrate(lox_t *db, const char *name, uint16_t old_v, uint16_t new_v) {
  (void)db;
  (void)name;
  g_migrate_calls++;
  g_migrate_old = old_v;
...
static lox_err_t db_open(bool wipe, bool with_mig) {
  lox_cfg_t cfg;
...
static bool run_kv_bench(void) {
  ...
  memset(&st, 0, sizeof(st));
  if (lox_inspect(&g_db, &st) != LOX_OK || st.kv_entries_max == 0u) return false;
  ...
  for (i = 0u; i < ops; ++i) {
    ...
    v = i + 1u;
    if (lox_kv_put(&g_db, key, &v, sizeof(v)) != LOX_OK) return false;
  ...
  for (i = 0u; i < ops; ++i) {
    ...
    out = 0u;
    out_len = 0u;
    if (lox_kv_get(&g_db, key, &out, sizeof(out), &out_len) != LOX_OK) return false;
  ...
  for (i = 0u; i < ops; ++i) {
    uint32_t t0 = micros();
    snprintf(key, sizeof(key), "k%05lu", (unsigned long)i);
    if (lox_kv_del(&g_db, key) != LOX_OK) return false;
    maybe_apply_write_control(i);
    {
      uint32_t dt = micros() - t0;
      total += dt;
...
static bool run_ts_bench(void) {
  ...
  uint32_t heap0, heap1;
  uint32_t value;
  lox_ts_sample_t out_buf[64];
  size_t retained = 0u;
  size_t out_count = 0u;
  lox_err_t e;

  e = lox_ts_register(&g_db, "temp", LOX_TS_U32, 0u);
  if (e != LOX_OK && e != LOX_ERR_EXISTS) return false;
  (void)lox_ts_clear(&g_db, "temp");

  heap0 = heap_free_8bit();
  for (i = 0u; i < ops; ++i) {
    uint32_t t0 = micros();
    value = i;
    if (lox_ts_insert(&g_db, "temp", i, &value) != LOX_OK) return false;
    maybe_apply_write_control(i);
    {
      uint32_t dt = micros() - t0;
      total += dt;
  ...
  {
    uint32_t t0 = micros();
    if (lox_ts_query_buf(&g_db, "temp", 0u, (lox_timestamp_t)ops, out_buf, 64u, &out_count) != LOX_OK && out_count == 0u)
      return false;
    g_lat[0] = micros() - t0;
  }
  emit_metric("ts_query_buf", 1u, g_lat[0], out_count * sizeof(lox_ts_sample_t), g_lat, 1u, 1u, heap_free_8bit(), heap_free_8bit());
  return true;
}
...
static bool run_rel_bench(void) {
  ...
  if (lox_schema_init(&schema, "bench_rel", rows + 16u) != LOX_OK) return false;
  schema.schema_version = 1u;
  if (lox_schema_add(&schema, "id", LOX_COL_U32, sizeof(uint32_t), true) != LOX_OK) return false;
  if (lox_schema_add(&schema, "temp", LOX_COL_U16, sizeof(uint16_t), false) != LOX_OK) return false;
  if (lox_schema_seal(&schema) != LOX_OK) return false;

  e = lox_table_create(&g_db, &schema);
  if (e != LOX_OK && e != LOX_ERR_EXISTS) return false;
  if (lox_table_get(&g_db, "bench_rel", &table) != LOX_OK) return false;
  if (lox_rel_clear(&g_db, table) != LOX_OK) return false;

  /* Isolate REL timing from prior stage WAL pressure in deterministic mode. */
  if (is_deterministic_profile() && lox_flush(&g_db) != LOX_OK) return false;
  ...
  for (i = 0u; i < rows; ++i) {
    ...
    maybe_apply_write_control(i);
    {
      uint32_t dt = micros() - t0;
      total += dt;
      bytes += lox_table_row_size(table);
      if (i < 64u) {
        cold_total += dt;
        cold_ops++;
      } else {
  ...
  if (lox_rel_count(table, &count_rows) != LOX_OK) return false;
...
static bool run_wal_compact_bench(void) {
  ...
  if (lox_compact(&g_db) != LOX_OK) return false;
  if (lox_inspect(&g_db, &start) != LOX_OK) return false;
  ...
  if (lox_kv_put(&g_db, kProbeKey, probe_before, sizeof(probe_before)) != LOX_OK) return false;
  ...
  for (i = 0u; i < max_ops; ++i) {
    ...
    if (lox_kv_put(&g_db, key, val, val_bytes) != LOX_OK) return false;
    maybe_apply_write_control(i);
    dt = micros() - t0;
    total += dt;
    bytes += val_bytes;
    ...
    if (((ops_done % 64u) == 0u) || (ops_done == max_ops)) {
      if (lox_inspect(&g_db, &before) != LOX_OK) return false;
  ...
  if (lox_inspect(&g_db, &after) != LOX_OK) return false;
  ...
  if (lox_kv_get(&g_db, kProbeKey, probe_after, sizeof(probe_after), &probe_len) != LOX_OK) return false;
...
static bool run_reopen_check(void) {
  lox_stats_t st;
  if (lox_deinit(&g_db) != LOX_OK) return false;
  {
    uint32_t t0 = micros();
    if (db_open(false, false) != LOX_OK) return false;
    g_lat[0] = micros() - t0;
  }
  if (lox_inspect(&g_db, &st) != LOX_OK) return false;
...
static bool run_migration_check(void) {
  ...
  memset(&schema, 0, sizeof(schema));
  if (lox_schema_init(&schema, "migr_tbl", 16u) != LOX_OK) return false;
  schema.schema_version = 1u;
  if (lox_schema_add(&schema, "id", LOX_COL_U32, sizeof(uint32_t), true) != LOX_OK) return false;
  if (lox_schema_seal(&schema) != LOX_OK) return false;
  if (lox_table_create(&g_db, &schema) != LOX_OK) return false;
  if (lox_table_get(&g_db, "migr_tbl", &table) != LOX_OK) return false;
  (void)table;

  if (lox_deinit(&g_db) != LOX_OK) return false;
  if (db_open(false, true) != LOX_OK) return false;
...
static bool run_txn_check(void) {
  ...
  if (lox_txn_begin(&g_db) != LOX_OK) return false;
  if (lox_kv_put(&g_db, "txn_a", &v1, sizeof(v1)) != LOX_OK) return false;
  if (lox_txn_commit(&g_db) != LOX_OK) return false;
  if (lox_kv_get(&g_db, "txn_a", &out, sizeof(out), NULL) != LOX_OK || out != v1) return false;

  if (lox_txn_begin(&g_db) != LOX_OK) return false;
  if (lox_kv_put(&g_db, "txn_a", &v2, sizeof(v2)) != LOX_OK) return false;
  if (lox_txn_rollback(&g_db) != LOX_OK) return false;
  out = 0u;
  if (lox_kv_get(&g_db, "txn_a", &out, sizeof(out), NULL) != LOX_OK || out != v1) return false;
  return true;
}
...
static bool run_real_data_suite(void) {
  ...
  lox_ie_options_t ie_opts = lox_ie_default_options();
...
#define RD_CHECK_REAL(label, expr)                                                                                                 \
  do {                                                                                                                             \
    ...
    MDB_CONSOLE.printf("[RD][%-30s] rc=%s (%d) %lu us\n", (label), lox_err_to_string(_rc), (int)_rc, (unsigned long)_dt);   \
    if (_rc != LOX_OK) {                                                                                                       \
      first_fail = (label);                                                                                                        \
      goto rd_fail;                                                                                                                \
    }                                                                                                                              \
...
#define RD_EXPECT_REAL(label, cond)                                                                                                \
  ...
  RD_CHECK_REAL("kv_put/wifi.ssid", lox_kv_put(&g_db, "wifi.ssid", "HomeNetwork_5G", 14u));
  RD_CHECK_REAL("kv_set/interval", lox_kv_set(&g_db, "sensor.interval_ms", &v_5000, sizeof(uint32_t), 0u));
  RD_CHECK_REAL("kv_set/boot.count", lox_kv_set(&g_db, "boot.count", &v_1, sizeof(uint32_t), 2u));
  RD_CHECK_REAL("kv_get/interval", lox_kv_get(&g_db, "sensor.interval_ms", &u32, sizeof(u32), &out_len));
  RD_EXPECT_REAL("assert/interval", u32 == 5000u && out_len == sizeof(uint32_t));
  RD_CHECK_REAL("kv_del/boot.count", lox_kv_del(&g_db, "boot.count"));
  RD_CHECK_REAL("admit_kv_set", lox_admit_kv_set(&g_db, "wifi.ssid", 16u, &adm));
  RD_CHECK_REAL("json/set_u32", lox_json_kv_set_u32(&g_db, "json.counter", 9876u, 0u));
  RD_CHECK_REAL("json/get_u32", lox_json_kv_get_u32(&g_db, "json.counter", &u32));
  ...
  RD_CHECK_REAL("kv_del/wifi.ssid", lox_kv_del(&g_db, "wifi.ssid"));
  RD_CHECK_REAL("kv_del/sensor.interval", lox_kv_del(&g_db, "sensor.interval_ms"));
  RD_CHECK_REAL("kv_del/json.counter", lox_kv_del(&g_db, "json.counter"));
  RD_CHECK_REAL("ie/import_kv", lox_ie_import_kv_json(&g_db, ie_json, &ie_opts, &ie_imported, &ie_skipped));
  RD_EXPECT_REAL("assert/ie.imported", ie_imported == 3u && ie_skipped == 0u);
  RD_CHECK_REAL("json/get_u32/reimport", lox_json_kv_get_u32(&g_db, "json.counter", &u32));
  RD_EXPECT_REAL("assert/json.reimport", u32 == 9876u);

  RD_CHECK_REAL("ts_register/temp", lox_ts_register(&g_db, "temperature", LOX_TS_F32, 0u));
  RD_CHECK_REAL("ts_insert/t1", lox_ts_insert(&g_db, "temperature", 1700000000u, &tf1));
  RD_CHECK_REAL("ts_insert/t2", lox_ts_insert(&g_db, "temperature", 1700000120u, &tf2));
  RD_CHECK_REAL("ts_last/temp", lox_ts_last(&g_db, "temperature", &ts_last));
  RD_EXPECT_REAL("assert/ts_last", ts_last.ts == 1700000120u);
  RD_CHECK_REAL("ts_count/temp", lox_ts_count(&g_db, "temperature", 0u, (lox_timestamp_t)0xFFFFFFFFu, &ts_count));
  RD_EXPECT_REAL("assert/ts_count", ts_count >= 2u);

  memset(&schema, 0, sizeof(schema));
  RD_CHECK_REAL("rel_schema_init", lox_schema_init(&schema, "event_log", 16u));
  RD_CHECK_REAL("rel_schema_add/id", lox_schema_add(&schema, "id", LOX_COL_U32, sizeof(uint32_t), true));
  RD_CHECK_REAL("rel_schema_add/sev", lox_schema_add(&schema, "severity", LOX_COL_U8, sizeof(uint8_t), false));
  RD_CHECK_REAL("rel_schema_seal", lox_schema_seal(&schema));
  {
    lox_err_t rc = lox_table_create(&g_db, &schema);
    MDB_CONSOLE.printf("[RD][%-30s] rc=%s (%d)\n", "rel_table_create", lox_err_to_string(rc), (int)rc);
  ...
  RD_CHECK_REAL("txn_begin", lox_txn_begin(&g_db));
  RD_CHECK_REAL("txn_set/a", lox_kv_set(&g_db, "txn.a", &v_100, sizeof(uint32_t), 0u));
  RD_CHECK_REAL("txn_commit", lox_txn_commit(&g_db));
  RD_CHECK_REAL("txn_begin2", lox_txn_begin(&g_db));
  RD_CHECK_REAL("txn_set/undo", lox_kv_set(&g_db, "txn.undo", &v_999, sizeof(uint32_t), 0u));
  RD_CHECK_REAL("txn_rollback", lox_txn_rollback(&g_db));

  RD_CHECK_REAL("flush", lox_flush(&g_db));
  RD_CHECK_REAL("deinit", lox_deinit(&g_db));
  RD_CHECK_REAL("reinit", db_open(false, false));
  RD_CHECK_REAL("recover/kv_get", lox_kv_get(&g_db, "wifi.ssid", row, sizeof(row), &out_len));
  RD_CHECK_REAL("recover/ts_count", lox_ts_count(&g_db, "temperature", 0u, (lox_timestamp_t)0xFFFFFFFFu, &ts_count));
  RD_CHECK_REAL("db_stats", lox_get_db_stats(&g_db, &dbs));
  RD_CHECK_REAL("kv_stats", lox_get_kv_stats(&g_db, &kvs));
  RD_CHECK_REAL("ts_stats", lox_get_ts_stats(&g_db, &tss));
  RD_CHECK_REAL("rel_stats", lox_get_rel_stats(&g_db, &rs));
  RD_CHECK_REAL("eff_cap", lox_get_effective_capacity(&g_db, &ec));
  RD_CHECK_REAL("pressure", lox_get_pressure(&g_db, &p));
...
static void run_full_bench_once(void) {
  ...
  clear_metrics();
  MDB_CONSOLE.println();
  MDB_CONSOLE.printf("=== loxdb ESP32-S3 benchmark start (profile=%s) ===\n", P()->name);

  deinit_rc = lox_deinit(&g_db);
  open_rc = db_open(true, false);
  if (deinit_rc != LOX_OK || open_rc != LOX_OK) {
    MDB_CONSOLE.printf("[ERR] pre-run reset/open failed: deinit=%s (%d), open=%s (%d)\n",
                       lox_err_to_string(deinit_rc),
                       (int)deinit_rc,
                       lox_err_to_string(open_rc),
                       (int)open_rc);
    return;
  }
  print_effective_capacity();
  ...
  if (!ok) return;
  if (stage_flush) (void)lox_flush(&g_db);
  ok = run_ts_bench();
  ...
  MDB_CONSOLE.println("=== loxdb ESP32-S3 benchmark end ===");
}
...
static void reset_db_and_open(bool wipe) {
  ...
  if (d != LOX_OK) MDB_CONSOLE.printf("[WARN] deinit returned %d\n", (int)d);
  {
    lox_err_t o = db_open(wipe, false);
    if (o != LOX_OK)
      MDB_CONSOLE.printf("[ERR] db_open failed: %s (%d)\n", lox_err_to_string(o), (int)o);
    else
...
void setup(void) {
  ...
  if (err != LOX_OK) {
    MDB_CONSOLE.printf("[FATAL] lox_init failed: %s (%d)\n", lox_err_to_string(err), (int)err);
    return;
  }

  MDB_CONSOLE.println();
  MDB_CONSOLE.println("loxdb ESP32-S3 terminal bench is ready.");
