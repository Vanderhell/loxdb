...
try {
    ...
    $buf = ""
    $ready = Read-UntilPattern -Serial $serial -Pattern "microdb-bench>" -TimeoutSec $OpenTimeoutSec -Buffer ([ref]$buf)
    $fullLog += $buf
    if (-not $ready) {
        throw "Prompt 'microdb-bench>' not detected on $Port within $OpenTimeoutSec s."
    }
    ...
    foreach ($cmd in $commands) {
        ...
        if ($cmd -eq "run" -or $cmd -eq "run_det" -or $cmd -eq "run_det_paced") {
            $matched = ""
            $runDone = Read-UntilAnyPattern -Serial $serial -Patterns @("=== microdb ESP32-S3 benchmark end ===", "microdb-bench>") -TimeoutSec $RunTimeoutSec -Buffer ([ref]$buf) -MatchedPattern ([ref]$matched)
            $fullLog += $buf
            if (-not $runDone) {
                throw "Benchmark command '$cmd' did not finish within $RunTimeoutSec s."
            }
            if ($matched -eq "microdb-bench>" -and $buf -notmatch [regex]::Escape("=== microdb ESP32-S3 benchmark end ===")) {
                throw "Benchmark command '$cmd' returned to prompt without benchmark end marker."
            }
            if ($buf -notmatch [regex]::Escape("microdb-bench>")) {
                $buf = ""
                $promptBack = Read-UntilPattern -Serial $serial -Pattern "microdb-bench>" -TimeoutSec 20 -Buffer ([ref]$buf)
                $fullLog += $buf
                if (-not $promptBack) {
            ...
            $okBack = Read-UntilPattern -Serial $serial -Pattern "microdb-bench>" -TimeoutSec 30 -Buffer ([ref]$buf)
            $fullLog += $buf
            if (-not $okBack) {
...
if ($ok) {
    Write-Host ""
    Write-Host "microdb benchmark on ${Port}: PASS"
    exit 0
}
