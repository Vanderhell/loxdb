#line 1 "C:\\Users\\vande\\Desktop\\github\\microdb\\bench\\microdb_esp32_s3_bench\\src\\microdb_json_wrapper.c"
...
static microdb_err_t json_append_char(char *out, size_t out_len, size_t *pos, char c) {
...
static microdb_err_t json_append_str(char *out, size_t out_len, size_t *pos, const char *s) {
    size_t n = strlen(s);
...
static microdb_err_t json_append_escaped(char *out, size_t out_len, size_t *pos, const char *s) {
...
static microdb_err_t json_append_hex(char *out, size_t out_len, size_t *pos, const uint8_t *buf, size_t len) {
    static const char hx[] = "0123456789ABCDEF";
    size_t i;
...
static microdb_err_t json_parse_u32(const char **p, uint32_t *out) {
    unsigned long v;
    char *end = NULL;
    json_skip_ws(p);
...
static microdb_err_t json_parse_string(const char **p, char *out, size_t out_len) {
    size_t pos = 0u;
    json_skip_ws(p);
...
static microdb_err_t json_parse_hex_string(const char **p, uint8_t *out, size_t out_len, size_t *out_len_used) {
    size_t n = 0u;
    json_skip_ws(p);
    if (**p != '"') return MICRODB_ERR_INVALID;
    (*p)++;
    while (**p != '\0' && **p != '"') {
        if (!json_is_hex_char((*p)[0]) || !json_is_hex_char((*p)[1])) return MICRODB_ERR_INVALID;
...
microdb_err_t microdb_json_kv_set_u32(microdb_t *db, const char *key, uint32_t value, uint32_t ttl) {
    ...
    return microdb_kv_set(db, key, buf, (size_t)n, ttl);
}
...
microdb_err_t microdb_json_kv_get_u32(microdb_t *db, const char *key, uint32_t *out_value) {
    ...
    unsigned long v;
    microdb_err_t rc;
    if (out_value == NULL) return MICRODB_ERR_INVALID;
    rc = microdb_kv_get(db, key, buf, sizeof(buf) - 1u, &out_len);
    if (rc != MICRODB_OK) return rc;
    buf[out_len] = '\0';
    v = strtoul(buf, &end, 10);
    if (end == NULL || *end != '\0' || v > 0xFFFFFFFFul) return MICRODB_ERR_INVALID;
...
microdb_err_t microdb_json_kv_get_i32(microdb_t *db, const char *key, int32_t *out_value) {
    ...
    buf[out_len] = '\0';
    v = strtol(buf, &end, 10);
    if (end == NULL || *end != '\0' || v < (-2147483647L - 1L) || v > 2147483647L) return MICRODB_ERR_INVALID;
    *out_value = (int32_t)v;
    return MICRODB_OK;
}
...
microdb_err_t microdb_json_kv_set_bool(microdb_t *db, const char *key, bool value, uint32_t ttl) {
    ...
    return microdb_kv_set(db, key, s, strlen(s), ttl);
}
...
microdb_err_t microdb_json_kv_set_cstr(microdb_t *db, const char *key, const char *value, uint32_t ttl) {
    if (value == NULL) return MICRODB_ERR_INVALID;
    return microdb_kv_set(db, key, value, strlen(value), ttl);
}

microdb_err_t microdb_json_kv_get_cstr(microdb_t *db, const char *key, char *out_buf, size_t out_buf_len, size_t *out_len) {
    microdb_err_t rc;
    size_t n = 0u;
    if (out_buf == NULL || out_buf_len == 0u) return MICRODB_ERR_INVALID;
    rc = microdb_kv_get(db, key, out_buf, out_buf_len - 1u, &n);
    if (rc != MICRODB_OK) return rc;
    out_buf[n] = '\0';
...
microdb_err_t microdb_json_encode_kv_record(const char *key,
    ...
    size_t pos = 0u;
    microdb_err_t rc;
    if (key == NULL || value == NULL || out_json == NULL || out_json_len == 0u || out_used == NULL) return MICRODB_ERR_INVALID;
    ...
    if (rc != MICRODB_OK) return rc;
    {
        ...
        if (n <= 0 || (size_t)n >= sizeof(num)) return MICRODB_ERR_INVALID;
...
microdb_err_t microdb_json_decode_kv_record(const char *json,
    ...
    if (!(seen_key && seen_ttl && seen_val)) return MICRODB_ERR_INVALID;
    return MICRODB_OK;
}
