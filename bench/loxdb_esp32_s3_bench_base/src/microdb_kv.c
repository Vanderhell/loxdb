...
static uint32_t lox_kv_live_value_bytes(const lox_core_t *core) {
    ...
    for (i = 0; i < core->kv.bucket_count; ++i) {
        if (core->kv.buckets[i].state == LOX_KV_BUCKET_LIVE) {
...
static lox_err_t lox_kv_find_slot(lox_core_t *core, const char *key, uint32_t *slot_out, bool *found_out) {
    uint32_t mask = core->kv.bucket_count - 1u;
    uint32_t idx = lox_kv_hash(key) & mask;
    ...
    for (probed = 0; probed < core->kv.bucket_count; ++probed) {
        ...
        } else if (strncmp(bucket->key, key, LOX_KV_KEY_MAX_LEN) == 0) {
            *slot_out = idx;
            *found_out = true;
            return LOX_OK;
        } else {
...
static lox_err_t lox_kv_set_at_internal(lox_t *db,
    ...
    err = lox_kv_find_slot(core, key, &slot, &found);
...
lox_err_t lox_txn_commit(lox_t *db) {
    ...
    for (i = 0u; i < core->txn_stage_count; ++i) {
        ...
        if (entry->op == LOX_TXN_OP_PUT) {
            rc = lox_persist_kv_set_txn(db, entry->key, entry->val_ptr, entry->val_len, 0u);
            if (rc != LOX_OK) {
                goto unlock;
            }
        } else {
    ...
    for (i = 0u; i < core->txn_stage_count; ++i) {
        lox_txn_stage_entry_t *entry = &core->txn_stage[i];
        if (entry->op == LOX_TXN_OP_PUT) {
            rc = lox_kv_set_at_internal(db, entry->key, entry->val_ptr, entry->val_len, 0u, false);
            if (rc != LOX_OK) {
                goto unlock;
            }
        } else {
