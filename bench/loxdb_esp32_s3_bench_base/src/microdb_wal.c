...
static lox_err_t lox_storage_write_bytes(lox_core_t *core, uint32_t offset, const void *buf, size_t len) {
    lox_err_t err = core->storage->write(core->storage->ctx, offset, buf, len);
    if (err == LOX_OK) {
...
static lox_err_t lox_write_ts_page(lox_core_t *core, uint32_t bank, uint32_t generation) {
    ...
    for (i = 0u; i < LOX_TS_MAX_STREAMS; ++i) {
        ...
        for (j = 0u; j < stream->count; ++j) {
            const lox_ts_sample_t *sample = &stream->buf[idx];
            ...
            err = lox_storage_write_bytes(core, offset, &sample->v, val_len);
            if (err != LOX_OK) {
                return err;
            }
            crc = lox_crc32(crc, &sample->v, val_len);
            offset += val_len;
...
static lox_err_t lox_replay_wal(lox_t *db, bool *out_had_entries, bool *out_header_reset) {
    ...
    *out_header_reset = false;
    ...
    if (lox_get_u32(header + 0u) != LOX_WAL_MAGIC) {
        LOX_LOG("ERROR", "WAL header corrupt: resetting WAL");
...
lox_err_t lox_storage_bootstrap(lox_t *db) {
...

    {
        ...
        core->layout.ts_size = lox_align_u32((uint32_t)core->ts_arena.capacity + LOX_PAGE_HEADER_SIZE, erase_size);
        core->layout.rel_size = lox_align_u32((uint32_t)core->rel_arena.capacity + LOX_PAGE_HEADER_SIZE, erase_size);
    ...
    return lox_reset_wal(core, core->wal_sequence);
}
...
static lox_err_t lox_append_wal_entry(lox_t *db,
    ...
    lox_put_u32(entry + 0u, LOX_WAL_ENTRY_MAGIC);
    lox_put_u32(entry + 4u, core->wal_entry_count + 1u);
    entry[8] = engine;
    entry[9] = op;
    lox_put_u16(entry + 10u, payload_len);
    if (payload_len != 0u) {
        memcpy(entry + 16u, payload, payload_len);
    }
    crc = LOX_CRC32(entry, 12u);
    crc = lox_crc32(crc, entry + 16u, payload_len);
    lox_put_u32(entry + 12u, crc);

    err = lox_storage_write_bytes(core, core->layout.wal_offset + core->wal_used, entry, entry_len);
    if (err != LOX_OK) {
        return err;
    }
...
static lox_err_t lox_compact_nolock(lox_t *db) {
    ...
    if (core->wal_enabled) {
        return lox_reset_wal(core, core->wal_sequence + 1u);
    }
    return LOX_OK;
}
